...
Automated coverage is handled entirely by the Node.js test runner. To execute the suite:

1. Install [Node.js 20+](https://nodejs.org/) so the built-in test runner and modern DOM APIs are available.
...
The command above loads the inline script from `exerciseTimer2.html` inside a mocked DOM (see `tests/helpers/createDom.js`) and executes:

- **Unit tests** in `tests/unit/workout.unit.test.js`, which validate helper logic such as drag-and-drop ordering, workout serialization, and timer state transitions.
- **End-to-end tests** in `tests/e2e/workout.e2e.test.js`, which step through an entire workout flow—adding exercises, starting the timer, simulating countdowns, and verifying UI mutations.

To focus on a specific file while iterating, you can invoke the runner directly:

```bash
node --test tests/unit/workout.unit.test.js
```

Because the suite relies on Node-only tooling, no browser is required for verification, enabling fast feedback during development.
