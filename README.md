...
Automated coverage lives entirely in the repository—no external services or browsers are required. The project uses the built-in Node.js test runner so the only prerequisite is a modern Node runtime.

### Prerequisites

1. Install [Node.js 20+](https://nodejs.org/) so the standard library DOM APIs and the `node --test` runner are available.
2. No packages need to be installed, but running `npm install` will generate a local lockfile if you prefer to track tool versions explicitly.
...
The script loads the inline JavaScript from `exerciseTimer2.html` inside a mocked DOM (see `tests/helpers/createDom.js`) and executes both layers of coverage:

- **Unit tests** (`tests/unit/workout.unit.test.js`) exercise the pure helpers: drag-and-drop ordering, workout serialization, timer state transitions, and speech fallbacks.
- **End-to-end tests** (`tests/e2e/workout.e2e.test.js`) simulate a user session—adding exercises, starting the timer, advancing countdowns, and asserting DOM updates.

### Targeted or watch runs

While iterating you can point the runner at a single file:

```bash
node --test tests/unit/workout.unit.test.js
```

For a live re-run loop, append `--watch` (requires Node 20.2+):

```bash
node --test --watch tests/e2e/workout.e2e.test.js
```

Because the suite runs entirely under Node, no browser automation is required—tests provide fast feedback directly from the command line.
